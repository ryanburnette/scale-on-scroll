...
function bigStartOffset() {
  options.startOffset = Number.MAX_VALUE
}

function addStickyCSS() {
  require('../node_modules/sticky-events/sticky-events.css')
}
...
function addStickyEventListeners() {
  console.log('listening')
  options.element.classList.add('sticky-events')
  observeStickyEvents()
  options.element.addEventListener(StickyEvent.STUCK, (event) => {
    console.log('stuck')
    options.startOffset = windowScrollPosition(options.context)
  })

  options.element.addEventListener(StickyEvent.UNSTUCK, (event) => {
    console.log('unstuck')
    bigStartOffset()
  })
}
